...
class DataBaseManager(object):
    ...
    def __init__(self):
        ...
        管理员工信息实例初始化：
            1. 连接数据库 -> people
            2. 选择数据集合 -> personal_info
            3. 生成操作数据集合的操作字典（菜单） -> method_dict
            4. 运行用户交互 -> self.run()
        '''
        client = MongoClient() # 连接 数据库服务器
        database = client.people # 选择 people 这个数据库
        self.col = database.personal_info # 选择 personal_info 这个集合
    ...
    def getInput(self):
        '''
        获取用户操作信息
        :return: string command 去除前后空格的字符串
        '''
    ...
    def analysis(self, command):
        ...
        分析输入的命令，执行对应的操作:
            命令是一个以 空格 区分的字符串，因此需要使用 split 将命令切分成一个 列表
                第0个元素代表命令，后面的元素是参数
        '''
        # command_list = command.split(' ') # 这个方法不适合于当命令中参数之间存在多个空格的情况
    ...
    def add_person_info(self, para_dict):
        '''
        添加人员信息，由于id没有使用 MongoDB 的_id,所以需要自行判断是否重复。
        id是必需的信息，所以这个方法也会坚持添加的信息中是否有id
        '''
        def check_if_id_exists(person_id):
            '''
            笨办法遍历数据库，看新增的这个id是否已经存在，如果不存在就返回False，不存在就返回False
            '''
